...
- 执行 Maintenance Loop 前，先读取 `.agent/loop.md`、`.agent/rules.md`、`.agent/memory.md` 与最近的 `.agent/history.md`，以仓库当前内容和新证据为准。
- 用户可见输出使用简体中文。
...
- 同一模块可以继续优化，但必须基于新的证据（新失败、新复现、新指标或新用户反馈）；不得重复处理 `.agent/history.md` 与 `.agent/archive/` 归档中已完成的问题。
...
- 维护历史记录时保留最近约 10 条，旧条目原样归档到 `.agent/archive/`；不得删除、改写或截断归档条目。
...
- 覆盖用户修改、删除用户数据、执行破坏性命令，或读取/修改凭证、私密数据和无关项目。
